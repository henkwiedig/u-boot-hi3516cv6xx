...
#define uapi_align_4(x)        	 ((unsigned int)((x) + 0x3) & (~0x3)) /* construct 4-byte address alignment */
#define uapi_is_align_u32(x)   	 (!((x) & 3))                         /* check for 4-byte alignment */
#define uapi_is_unalign_u32(x)   ((x) & 3)                            /* check for 4-byte alignment */

#if defined(HAVE_PCLINT_CHECK)
#define uapi_fieldoffset(s, m)   (0) /* member offset in a structure */
#else
#define uapi_fieldoffset(s, m)   ((td_u32) & (((s*)0)->m)) /* member offset in a structure */
#endif
...
#define uapi_reg_clrbits(addr, pos, bits)      (uapi_reg_read_val32(addr) &= ~((((unsigned int)1 << \
    (bits)) - 1) << (pos)))
...
#define uapi_reg_getbits(addr, pos, bits)      ((uapi_reg_read_val32(addr) >> (pos)) & (((unsigned int)1 << \
    (bits)) - 1))
...
#define uapi_reg_clrbits16(addr, pos, bits) (uapi_reg_read_val16(addr) &= ~((((unsigned short)1 << (bits)) - 1) << \
    (pos)))
