...
#if !!defined(CONFIG_MMU_SUPPORT)
    g_node_buffer = crypto_malloc_mmz(HASH_NODE_SIZE * CONFIG_HASH_HARD_CHN_CNT, "CRYPTO HASH Node Buffer");
    ...
    in_node_type_e in_node_type)
{
    ...
    crypto_chk_return(data_phys % CONFIG_HASH_ADDR_ALIGN_LEN != 0, HASH_COMPAT_ERRNO(ERROR_HASH_ADDR_NOT_ALIGNED),
        "data_phys must be aligned to %u-byte\n", CONFIG_HASH_ADDR_ALIGN_LEN);
    hard_ctx = &g_hash_hard_ctx[chn_num];
