    ...
    const drv_pke_ecc_point *q, const drv_pke_data *x)
{
    ...
    unsigned char g_x_jac[DRV_PKE_LEN_576];
    unsigned char g_y_jac[DRV_PKE_LEN_576];
    unsigned char q_x_jac[DRV_PKE_LEN_576];
    unsigned char q_y_jac[DRV_PKE_LEN_576];
    if (memset_s(g_x_jac, sizeof(g_x_jac), 0, sizeof(g_x_jac) != 0)) {
        return ret;
    }
    if (memset_s(g_y_jac, sizeof(g_y_jac), 0, sizeof(g_y_jac) != 0)) {
        return ret;
    }
    if (memset_s(q_x_jac, sizeof(q_x_jac), 0, sizeof(q_x_jac) != 0)) {
        return ret;
    }
    if (memset_s(q_y_jac, sizeof(q_y_jac), 0, sizeof(q_y_jac) != 0)) {
        return ret;
    }
